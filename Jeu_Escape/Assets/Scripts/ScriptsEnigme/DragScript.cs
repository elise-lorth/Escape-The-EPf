...
public class DragScript : MonoBehaviour
{
	...
	void OnMouseDrag()
	{
		...
		if (GetComponent<Collider2D>() == Physics2D.OverlapPoint(mousePosition) && moveAllowed && thisColTouched)
        {
			// then object is moved to the touch position
			// corrected by the offset
			rb.MovePosition(new Vector2(mousePosition.x - offsetX, mousePosition.y - offsetY));
		}
